    ...
    fun createChannelDecorator(channel: Channel, effectDescriptor: String): ChannelDecorator {
        ...
        try {
            return when (effect[0]) {
                ...
                "ads" -> if (effect.size != 4) throw EffectParseException("invalid ADS parameter count") else ADSEnvelopeDecorator(
                    channel,
                    effect[1].toDouble(),
                    effect[2].toDouble(),
                    effect[3].toDouble()
            ...
            if (e !is EffectParseException) {
                throw EffectParseException("Invalid parameters: $effectDescriptor")
            } else {
                throw e
            }
        }
